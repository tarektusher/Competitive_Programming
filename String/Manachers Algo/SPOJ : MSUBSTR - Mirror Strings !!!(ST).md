# [Problem Link](https://www.spoj.com/problems/MSUBSTR/en/)
...
int main(){FIO;
    ...
    while(test--){
        ...
        for(ll i=0;i<n;i++)mx1=max(mx1,odd[i]),check1[odd[i]]++;
        //cout<<edl;
        for(ll i=0;i<n;i++)mx2=max(mx2,even[i]),check2[even[i]]++;
        if(mx1*2-1>mx2*2)cout<<mx1*2-1<<spc<<check1[mx1]<<endl;
        else if(mx1*2-1==mx2*2)cout<<mx1*2-1<<spc<<check1[mx1]+check2[mx2]<<edl;
        else cout<<mx2*2<<spc<<check2[mx2]<<endl;
    }
return 0;
}
