...
vector<ll>vc;
ll dp[35][2][2][35];
ll cal(ll pos,ll prev,ll f,ll sum){
    if(pos==vc.size())return sum;
    if(dp[pos][prev][f][sum] != -1)return dp[pos][prev][f][sum];
    ll res=0,LMT;
    if(f==0)LMT=vc[pos];
    else LMT=1;
    for(ll digit=0;digit<=LMT;digit++){
        ...
        res+=cal(pos+1,digit,nf,((digit== 1 && prev==1)+sum));
    }
    return dp[pos][prev][f][sum]=res;
}
ll solve(ll b){
    vc.clear();
    while(b){
