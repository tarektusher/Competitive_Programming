...
### [Problem 03](https://lightoj.com/problem/extreme-gcd)*[Here we find number of quadruples in an array which gcd is 1]*
### [Problem 04](https://codeforces.com/contest/803/problem/F)*[Here we find number of subsequences in an array which gcd is 1]*
...
int main(){FIO;
     ...
     while(test--){
          ...
          for(ll i=0;i<maX;i++){
               if(prdiv[i]%2==1){
                    cc=div[i]*(div[i]-1)*(div[i]-2)*(div[i]-3)/24;
                    bad+=cc;
               }
               else {
                    cc=div[i]*(div[i]-1)*(div[i]-2)*(div[i]-3)/24;
