/*
    Problem Link : https://lightoj.com/problem/karate-competition
...
int main(){FIO;
     ...
     while(test--){
          ll n;cin>>n;
          vector<ll>ara(n),str(n);
          for(ll i=0;i<n;i++)cin>>ara[i];
          for(ll i=0;i<n;i++)cin>>str[i];
          ll dp[n+1][n+1];
          sort(all(ara),greater<ll>());
          sort(all(str),greater<ll>());
          memset(dp,0,sizeof(dp));
          for(ll i=1;i<=n;i++){
               for(ll j=1;j<=n;j++){
                    if(str[j-1]>ara[i-1])dp[i][j]=dp[i-1][j];
                    else if(str[j-1]<ara[i-1])dp[i][j]=dp[i-1][j-1]+2;
                    else dp[i][j]=max(dp[i-1][j-1]+1,dp[i-1][j]);
               }
          }
          // for(ll i=1;i<=n;i++){
          //      for(ll j=1;j<=n;j++)cout<<dp[i][j]<<spc;
          //      cout<<edl;
          // }
          cout<<"Case "<<cs++<<": "<<dp[n][n]<<edl;
     }
return 0;
}
