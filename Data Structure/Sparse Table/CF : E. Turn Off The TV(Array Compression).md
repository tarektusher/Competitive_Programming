# [problem link](https://codeforces.com/contest/863/problem/E)
# Idea : Array Compression +Sparse Table
# Code 
```cpp
/*
    Problem Link : 
...
ll n;
map<ll,ll>check;
ll l[maX],r[maX],dif[maX],pre[maX];
ll sp[maX][25];

void build(ll cc){
     for(ll i=1;i<=cc;i++)sp[i][0]=dif[i];
     for(ll j=1;j<25;j++){
          for(ll i=1;i+(1<<j)-1<=cc;i++){
...
int main(){FIO;
     ...
     ll cc=0;
     for(auto &it: check)it.ss=++cc;
     for(ll i=1;i<=n;i++){
          dif[check[l[i]]]++;
          dif[check[r[i]+1]]--;
     }
     for(ll i=1;i<=cc;i++)dif[i]+=dif[i-1];
     build(cc);
     for(ll i=1;i<=n;i++){
          ll val=query(check[l[i]],check[r[i]]);
          if(val>mn)mn=val,ans=i;
     }
     cout<<ans<<edl;
return 0;
}
