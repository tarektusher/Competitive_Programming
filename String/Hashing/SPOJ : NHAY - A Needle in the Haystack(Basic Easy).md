...
int main(){FIO;
     ...
     while((cin>>n) && n != 0){
          string str,ara;
          cin>>str>>ara;
          if(str.size()>ara.size()){
               cout<<edl;
               cn;
          }
          ll hash=0;
          for(ll i=0;i<str.size();i++){
               hash=(hash+(str[i]-'a'+1)*po[i])%mod;
          }
          ll m=ara.size();
          vector<ll>hashtable(m+1);
          for(ll i=1;i<=m;i++){
               hashtable[i]=(hashtable[i-1]+(ara[i-1]-'a'+1)*po[i-1])%mod;
          }
          vector<ll>ans;
          for(ll i=n;i<=ara.size();i++){
               ll x=(hashtable[i]-hashtable[i-n]+mod)%mod;
               ll y=exp(po[i-n],mod-2);
               ll z=(x*y)%mod;
               if(z==hash){
                    ans.pb(i-n);
               }
          }
          for(auto it: ans)cout<<it<<edl;
          if(ans.size()==0)cout<<edl;
     }
return 0;
}
