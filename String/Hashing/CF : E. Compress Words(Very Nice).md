# [Problem Link](https://codeforces.com/problemset/problem/1200/E)
...
int main(){FIO;
    ...
    ans=a;
    ll n=a.size();
    for(ll test=1;test<q;test++){
        cin>>b; 
        ll m=b.size();
        n=ans.size();
        pair<ll,ll>hash1(0,0),hash2(0,0);
        ll cc=0;
        for(ll i=n-1,j=0;i>=0 && j<m;j++,i--){
            hash1.ff=(hash1.ff+b[j]*po[j])%mod;
            hash1.ss=(hash1.ss+b[j]*po1[j])%mod1;

            hash2.ff=(hash2.ff*p+ans[i])%mod;
            hash2.ss=(hash2.ss*pp+ans[i])%mod1;
            if(hash1==hash2){
                cc=j+1;
            }
        }
        for(ll i=cc;i<m;i++)ans+=b[i];
    }
    cout<<ans<<endl;
return 0;
}
