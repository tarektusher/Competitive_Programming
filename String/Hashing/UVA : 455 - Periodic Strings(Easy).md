# [Problem Link](https://vjudge.net/contest/434574#problem/L)
...
int main(){FIO;
    ...
    while(test--){
        ...
        for(ll i=0;i<n;i++){
            hash[i+1]=(hash[i]+(str[i])*po[i])%mod;
            //cout<<hash[i+1]<<spc;
        }
       // cout<<endl;
        for(ll i=1;i<=n;i++){
            if(n%i)cn;
            ll x=hash[n-i];
            ll y=(hash[n]-hash[i]+mod)%mod;
            y=(y*exp(po[i],mod-2))%mod;
            //cout<<i<<spc<<x<<spc<<y<<endl;
            if(x==y){
                cout<<i<<endl;
                break;
            }
