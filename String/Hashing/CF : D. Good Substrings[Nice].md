...
int main(){FIO;
    ...
    string str,ara;cin>>str>>ara;
    ll k;cin>>k;
    ll n=str.size();
    vector<ll>bad(n+1),hash(n+1);
    for(ll i=0;i<n;i++){
        hash[i+1]=(hash[i]*p+str[i]);
    }
    for(ll i=1;i<=n;i++){
        bad[i]=bad[i-1];
        if(ara[str[i-1]-'a']=='0')bad[i]++;
    }
    ll ans=0;
    for(ll i=1;i<=n;i++){
        for(ll j=i;j<=n;j++){
            if(bad[j]-bad[i-1]<=k){
                ll cc=hash[j]-hash[i-1]*po[j-i+1];
                if(check[cc]==0){
                    ans++;
                    check[cc]++;
                }
