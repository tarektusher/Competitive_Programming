...
vector<ll>vc;
ll dp[12][180][180][2],N;
ll a,b,k;
ll DP(ll pos,ll sum,ll rem,ll small){
    if(pos==N){
        if(sum%k==0 && rem%k==0)return 1;
        else return 0;
    }
    if(dp[pos][sum][rem][small] != -1)return dp[pos][sum][rem][small];
    ll ans=0,till;
    if(small)till=9;
    else till =vc[pos];
    for(ll i=0;i<=till ;i++){
        ...
        ans+=DP(pos+1,(sum+i)%k,x%k ,cc);
    }
    return dp[pos][sum][rem][small]=ans;
}
ll solve(ll n){
    vc.clear();
    while(n){
