...
int main(){FIO;
     ...
     ll capos=-1,smpos=-1;
     ll n=str.size();
     vector<ll>small(n+5,0),capital(n+5,0);
     for(ll i=1;i<=n;i++){
          small[i]=small[i-1];
          small[i]+=(str[i-1]>='a' && str[i-1]<='z');
     }
     for(ll i=n;i>0;i--){
          capital[i]=capital[i+1];
          capital[i]+=(str[i-1]>='A' && str[i-1]<='Z');
     }
     small[n+1]=small[n];
     capital[0]=capital[1];
 //    cout<<small[n]<<spc<<capital[1]<<edl;
     ll res=infinity;
     for(ll i=1;i<=n+1;i++){
          res=min(res,small[i-1]+capital[i]);
     }
     cout<<res<<edl;
return 0;
}
