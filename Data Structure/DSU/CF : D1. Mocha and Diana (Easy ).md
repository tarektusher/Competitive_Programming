...
ll par1[maX],sz1[maX],sz2[maX],par2[maX];
void Set(){
     for(ll i=1;i<maX;i++)par1[i]=i,sz1[i]=1,par2[i]=i,sz2[i]=1;
}
ll find(ll v,ll parent[]){
     if(v==parent[v])return v;
     return parent[v]=find(parent[v],parent);
}
void Union(ll u,ll v,ll parent[],ll sz[]){
     u=find(u,parent);
     v=find(v,parent);
     if(u != v){
...
int main(){FIO;
     ...
     for(ll i=1;i<=n;i++){
          for(ll j=i+1;j<=n;j++){
               if(find(i,par1)==find(j,par1) || find(i,par2)==find(j,par2))cn;
               ans.pb({i,j});
               Union(i,j,par1,sz1);
               Union(i,j,par2,sz2);
          }
     ...
     cout<<ans.size()<<edl;
     for(auto it : ans)cout<<it.ff<<spc<<it.ss<<edl;
return 0;
}
