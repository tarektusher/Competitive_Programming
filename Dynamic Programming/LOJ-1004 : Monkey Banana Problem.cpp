Porblem link : https://lightoj.com/problem/monkey-banana-problem
    ...
    while(test--){
        ...
        for(ll i=1;i<2*n;i++){
            if(i<=n){
                k=i;
                 for(ll j=1;j<=k;j++){
                    dp[i][j]=max(ara[i][j]+dp[i-1][j-1],ara[i][j]+dp[i-1][j]);
                   // cout<<dp[i][j]<<spc;
                }
                //cout<<endl;
            }
                ...
                k--;
                for(ll j=1;j<=k;j++){
                    dp[i][j]=max(dp[i-1][j]+ara[i][j],dp[i-1][j+1]+ara[i][j]);
                    //cout<<dp[i][j]<<spc;
                }
                //cout<<endl;
            }
