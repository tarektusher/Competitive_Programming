# [Problem Link](https://www.spoj.com/problems/SUFEQPRE/en/)
...
int main(){FIO;
    ...
    while(test--){
        ...
        f_hash.clear();
        for(ll i=1;i<=n;i++){
            f_hash[i]=(f_hash[i-1]+str[i-1]*po[i-1])%mod;
        }
        ll cc=0;
        for(ll i=2;i<=n;i++){
            ll hash=(f_hash[n]-f_hash[i-1]+mod)%mod;
            if((f_hash[n-i+1]*po[i-1])%mod==hash)cc++;
        }
        cout<<"Case "<<cs++<<": "<<cc<<edl;
    }
return 0;
