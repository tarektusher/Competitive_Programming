# [Problem Link](https://www.spoj.com/problems/CODERE3/)
## Idea : Take LIS from forward and from backward.It is also known as "Longest Bitonic Subsequence [Link](https://www.geeksforgeeks.org/longest-bitonic-subsequence-dp-15/)"<br>
# Code
```cpp
/*
    Problem Link : 
...
ll n;
vector<ll>vc;
vector<ll>dp(maX),dp1(maX);
void lca_right(){
     dp.clear();
     for(ll i=0;i<n;i++){
          dp[i]=1;
          for(ll j=0;j<i;j++){
               if(vc[i]>vc[j] && dp[i]<dp[j]+1){
                    dp[i]=dp[j]+1;
               }
          }
     }
...
int main(){FIO;     
     ...
     while(test--){
          ...
          ll cc=-1;
          for(ll i=0;i<n;i++){
               if(dp[i]+dp1[i]-1>cc)cc=dp[i]+dp1[i]-1;
          }
          cout<<cc<<edl;
     }
