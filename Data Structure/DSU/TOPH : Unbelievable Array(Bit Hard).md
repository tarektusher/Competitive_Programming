# [Problem Link](https://toph.co/p/unbelievable-array)
# Code
```cpp
/*
    Problem Link : 
...
ll ara[maX],parent[maX];
ll n,m;
ll col[maX],sz[maX],Find[maX],cnt[maX],id[maX];
void Set(){
     for(ll i=1;i<maX;i++)parent[i]=i,sz[i]=1,Find[i]=0,cnt[i]=0;
}
ll find(ll v){
...
int main(){FIO;
     ...
     while(test--){
          ...
          for(ll i=1;i<=n;i++){
               ...
               cnt[ara[i]]++;
               col[i]=ara[i];
               if(Find[col[i]]==0)Find[col[i]]=i;
               else{
                    Union(Find[col[i]],i);
               }
          }
          for(ll i=1;i<=n;i++)id[col[i]]=find(i);
          cout<<"Case "<<cs++<<":"<<edl;
          while(m--){
               ll temp;cin>>temp;
               if(temp==1){
